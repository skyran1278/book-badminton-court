...
const { courtIdAndTimes, courtIdNamePair, people } = require('./config');
    ...
    courtIdAndTimes.forEach(({ courtId, courtTime }) => {
      fetch(
        ...
        .then((body) => {
          ...
          console.log(
            /預約成功/.test(body)
              ? `${bookDate} ${person.account} ${courtIdNamePair[courtId]} ${courtTime} 預約成功 ${startTime} ${bookingTime}`
              : `${bookDate} ${person.account} ${courtIdNamePair[courtId]} ${courtTime} 預約失敗 ${startTime} ${bookingTime}`
          );
        });
    });
  });
